...
object Main {
    ...
    def main(args:Array[String]) : Unit = {
        if ( args.length < 1 ) {
            println("Usage: Teapot <triangles.txt> [threshold]")
        } else {
            ...
            val threshold = if ( args.length > 1 ) {
                ...
                println("calculated threshold = "+t)
                t
            }
            val t0 = System.currentTimeMillis
            val reducedTriangles = reduceTriangles(threshold, triangles)
            val dt = System.currentTimeMillis-t0
            println("nTriangles = "+(reducedTriangles length)+" in "+(dt/1000.0)+"s")
